---
title: "第 5 章：Poisson、Normal、Geometric、Negative Binomial、Gamma"
course: "统计学基础：数理统计"
---
...
flowchart TD
    ...
    %% Bernoulli -> Binomial -> Poisson -> Continuous
    A -->|n次独立重复试验成功次数| B["Binomial(n,p)"]
    B -->|n→∞，p→0，np=λ| C["Poisson(λ)"]
    C -->|事件发生间隔时间| D["Exponential(λ)<br/>= Gamma(1,λ)"]
    D -->|等待第 r 个事件| E["Gamma(r,λ)"]

    %% Bernoulli -> Geometric -> Negative Binomial
    A -->|重复Bernoulli试验到成功| F["Geometric(p)"]
    F -->|重复到第 r 次成功| G["Negative Binomial(r,p)"]
...
|  Gamma 参数一定是 scale   |     本章使用 rate 参数 $\lambda$      |
|      几何分布从 0 开始      |    本章定义为第几次成功，取值 $1,2,\dots$    |
|     负二项分布只数失败次数      | 不同教材定义不同，本章可理解为第 $r$ 次成功所需试验次数  |
...
E(X)=160.2,
\qquad
\sqrt{\operatorname{Var}(X)}=\sqrt{178(0.9)(0.1)}\approx4.00.
$$

座位足够的概率是 $P(X\le168)$。用正态近似时作连续性校正：

$$
P(X\le168)
\approx
\Phi\left(\frac{168.5-160.2}{4.00}\right)
\approx0.981.
$$
...
已知某指标在基准人群中服从 $N(\mu,\sigma^2)$，观测到 $x$ 后计算
...
- 这是在 $H_0$ 成立下的数据尾概率，不是 $H_0$ 为真的概率；
...
- $n$ 大、$p$ 很小且 $np=\lambda$ 适中：用 Poisson$(\lambda)$ 近似，关注稀有计数。
