---
title: "第 4 章：均值方差性质、顺序统计量、条件分布与矩母函数"
course: "统计学基础：数理统计"
---
...
|      条件密度忘记除以边际密度      | $f_{X\mid Y}(x\mid y)=\dfrac{f_{X,Y}(x,y)}{f_Y(y)}$ |
...
|   条件 pmf    |     $p_{X\mid Y}(x\mid y)=\dfrac{p_{X,Y}(x,y)}{p_Y(y)}$     |
...
一个常见题型是：设 $X\sim Bin(n,p)$，$Y\sim Bin(m,p)$ 且独立，给定 $X+Y=k$，求 $X$ 的条件分布。
...
设 $(U,V)=T(X,Y)$ 是一一可微变换，逆变换为 $(X,Y)=T^{-1}(U,V)$。则
...
$$
f_{XY}(u)=\int f_X(x)f_Y(u/x)\frac{dx}{|x|},
$$

$$
f_{X/Y}(v)=\int |y|f_X(vy)f_Y(y)\,dy.
$$
...
把 $n$ 封信随机放入 $n$ 个信封，令 $I_i$ 表示第 $i$ 封信放入正确信封，总正确数
...
$$
E(N)=\sum_iE(I_i)=n\cdot\frac1n=1.
$$

各 $I_i$ 并不独立，但期望线性性仍成立。这种技巧能绕开复杂的排列分布。

### 15.3 集成平均为何降低方差

设 $m$ 个预测器方差都为 $\sigma^2$，任意两个预测器相关系数为 $\rho$。平均预测

$$
\bar T=\frac1m\sum_{j=1}^mT_j
$$
...
若预测器独立，方差降为 $\sigma^2/m$；若高度相关，增加预测器数量的收益有限。因此随机森林不仅要平均很多树，还要通过样本和特征随机化降低树之间的相关性。
...
第二项非负，所以 $E(X\mid Y)$ 是用 $Y$ 预测 $X$ 的最优函数。全方差公式
