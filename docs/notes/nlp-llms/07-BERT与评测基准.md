...
- 能解释 BERT 输入三种 embedding 与特殊 token。
- 能写出 MLM 目标并说明 80/10/10 策略。
- 能区分 sequence、token、span 三种 fine-tuning 头。
- 能根据数据不平衡选择 accuracy、micro/macro-F1。
- 能指出 benchmark 分数之外至少三种有效性威胁。
