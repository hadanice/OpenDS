...
- 能由一组奖励手算标准化 advantage。
- 能比较 PPO critic baseline 与 GRPO group baseline。
- 能说明 group size、temperature、KL 系数的相互影响。
- 能设计一个同时含正确性与格式的奖励，并指出可被利用的漏洞。
- 能区分推理长度、可读解释与真实任务正确率。
