...
若 $Y\mid p\sim\mathrm{Bin}(n,p)$ 且 $p\sim\mathrm{Beta}(a,b)$，则

$$
p\mid Y=y\sim\mathrm{Beta}(a+y,b+n-y).
$$

先验中的 $a-1,b-1$ 可理解为成功与失败的伪计数，数据到来后直接累加。

若 $Y_1,\ldots,Y_n\mid\lambda\overset{i.i.d.}{\sim}\mathrm{Poisson}(\lambda)$，并采用 shape--rate 参数化 $\lambda\sim\mathrm{Gamma}(a,b)$，则

$$
\lambda\mid y
\sim
\mathrm{Gamma}\!\left(a+\sum_i y_i,\ b+n\right).
$$

若 $Y_i\mid\mu\sim N(\mu,\sigma^2)$、$\sigma^2$ 已知，且 $\mu\sim N(m_0,s_0^2)$，则后验精度与均值为

$$
\frac1{s_n^2}=\frac1{s_0^2}+\frac n{\sigma^2},
\qquad
...
=\arg\min_\beta
\left\{-\log L(\beta;y)-\log\pi(\beta)\right\}.
$$

在线性高斯模型中，独立高斯先验 $\beta_j\sim N(0,\tau^2)$ 产生 $\ell_2$ 惩罚，即 Ridge；独立 Laplace 先验产生 $\ell_1$ 惩罚，即 Lasso。正则化强度可解释为先验收缩强度，但频率学派中的调参和 Bayes 先验设定仍是不同的推理框架。

### 16.4 后验预测分布

对新观测 $Y_{\mathrm{new}}$，应把参数不确定性积分掉：
...
若 $T(Y)$ 对 $\theta$ 充分，则后验满足 $p(\theta\mid Y)=p(\theta\mid T(Y))$。这说明充分统计量不仅保留频率学派推断信息，也能把 Bayes 更新压缩为低维运算；但先验与似然不共轭时，后验仍可能需要数值积分、MCMC 或变分近似。
